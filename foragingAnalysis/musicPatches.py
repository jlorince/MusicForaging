...
class MPA(object):
    ...
    # init just takes in command line argumetns and sets up logging
    ...
    def patch_segmenter(self,df,metric,min_length,dist_thresh):
        ...
        for b in indices:
            dist = self.calc_dist(df.iloc[b]['features'],df.iloc[b-1]['features'], metric=metric)
    ...
    def processor(self,fi,output_dir,is_sorted=True,features=None,dist='cosine',session_threshold=None,dist_threshold=0.2,min_patch_length=5,artist_idx_feature_map=None):
        ...
        if features is not None:
            ...
            df['dist'] = df.apply(lambda row: self.calc_dist(row['features'],row['features_shift'],metric=dist),axis=1)
        ...
        if (min_patch_length is not None) and (dist_threshold is not None):
            ...
            df['block'] = ((df['artist_id'].shift(1) != df['artist_id']).astype(int).cumsum())-1

            self.rootLogger.debug('artist blocks done (user {})'.format(user))

        df.drop('features_shift',axis=1).to_pickle('{}{}.pkl'.format(output_dir,user))
    ...
    @staticmethod
    def patch_measures(df,agg_stats=True,metric='cosine'):
        first = df.iloc[0]
        n = len(df)
        start = first['ts']
        if agg_stats:
    ...
    def patch_summary(self,fi,basis,metric):
        ...
        result.to_pickle('{}{}_patches_{}.pkl'.format(self.args.pickledir,user,basis))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--session_thresh", help="session segmentation threshold",type=int,default=1800)
    parser.add_argument("--min_patch_length", help="minimum patch length",type=int,default=5)
    ...
    mpa = MPA(args,logging_level=logging.INFO)
    mpa.run()



 #python musicPatches.py --patch_basis patch_idx_shuffle --suppdir /Users/jaredlorince/git/MusicForaging/GenreModeling/data/ --pickledir /Users/jaredlorince/git/MusicForaging/testData/scrobbles_test/ -n 4 --feature_path /Users/jaredlorince/git/MusicForaging/GenreModeling/data/features/lda_artists/features_190.npy
