...
<<<<<<< HEAD
k_range = np.arange(10,201,5)
=======
k_range = np.arange(235,301,5)
...
def dist_prep(model,predict_data,token_counts):
    ...
    doc_topic_mat = np.array(model.predict(predict_data,output_type='probabilities'))
    word_topic_mat = np.array(model['topics']['topic_probabilities'])[seq]
...
if k_range[0]==10:
    ...
    last = np.load(d+'dists_'+str(k_range[0]-5)+'.npy')
...
=======
with open(d+'log','a') as log:
    #log.write('\t'.join(["K", "perplexity", "rank_corr_prev_cosine", "rank_corr_prev_euclidean","rank_corr_prev_jsd","rmse_prev_cosine", "rmse_prev_euclidean","rmse_prev_jsd","split_rank_corr_cosine", "split_rank_corr_euclidean", "split_rank_corr_jsd","split_rmse_cosine", "split_rmse_euclidean","split_rmse_jsd"])+'\n')
>>>>>>> 8cd66b95ea5dad24826289a07346f2ba280dcbc4
...
=======
            prev_comp_jsd = spearmanr(last[:,2],dists[:,2]).correlation
            prev_rmse_cosine = rmse(last[:,0],dists[:,0])
            prev_rmse_euclidean = rmse(last[:,1],dists[:,1])
            prev_rmse_jsd = rmse(last[:,2],dists[:,2])
>>>>>>> 8cd66b95ea5dad24826289a07346f2ba280dcbc4
...
=======
        split_comp_jsd = spearmanr(dists_a[:,2],dists_b[:,2]).correlation
        split_rmse_cosine = rmse(dists_a[:,0],dists_b[:,0])
        split_rmse_euclidean = rmse(dists_a[:,1],dists_b[:,1])
        split_rmse_jsd = rmse(dists_a[:,2],dists_b[:,2])

        print 'overall time for this test: %s' % str(datetime.timedelta(seconds=(time.time()-overall_start)))

        result = '\t'.join(map(str,[k,perplexity,prev_comp_cosine,prev_comp_euclidean,prev_comp_jsd,prev_rmse_cosine,prev_rmse_euclidean,prev_rmse_jsd,split_comp_cosine,split_comp_euclidean,split_comp_jsd,split_rmse_cosine,split_rmse_euclidean,split_rmse_jsd]))
>>>>>>> 8cd66b95ea5dad24826289a07346f2ba280dcbc4
        print result
        log.write(result+'\n')
        log.flush()
